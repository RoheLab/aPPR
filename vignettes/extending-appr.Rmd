...
pubmed_graph <- function(max_attempts = 3) {
  ...
  # TODO: separate outgoing citation types and incoming citation types?
...
check.pubmed <- function(graph, nodes) {
  ...
  # handle the case where only bad nodes are passed
  for (i in 1:graph$max_attempts) {
    ...
    logger::log_trace(glue::glue("Attempt {i}/{graph$max_attempts} to check nodes: {nodes}"))
    
    tryCatch({
      # suppressWarnings is to prevent the side effect of a warning being
      # printed to screen when some nodes are not found. we deal with this
      # is programmatically. be aware that this may suppress over important
      # warnings that we do not handle programmatically
      summ <- suppressWarnings(entrez_summary(db = "pubmed", id = nodes))
      break
    }, error = function(cnd) {
      # if all nodes are bad, summ isn't created and we have to handle that
      if (grepl("No esummary records", conditionMessage(cnd))) {
        logger::log_debug(glue::glue("Done checking nodes (all were bad)"))
        
        # would prefer to return(character(0)) here but that doesn't seem
        # to successfully return, so set a flag instead
        all_bad <- TRUE
      }
      
      if (i == graph$max_attempts){
        logger::log_debug(glue::glue("Maximum attempts to find neighborhood met, could not find: {nodes}"))
      }
    })
  }
    
  # todo: check if we can get node degrees for each of these suckers
  # todo: check if we can get the neighborhood for each of these suckers
  ...
  logger::log_debug(glue::glue("Done checking nodes (at least some were good)"))
...
node_degrees.pubmed <- function(graph, nodes) {
  ...
  for (i in 1:graph$max_attempts) {
    ...
    logger::log_trace(glue::glue("Attempt {i}/{graph$max_attempts} to get node degrees: {nodes}"))
    
    tryCatch({
      
      # TODO: i don't really trust this
      cites <- entrez_link(dbfrom = "pubmed", db = "all", id = nodes)
      break
    }, error = function(cnd) {
      if (i == graph$max_attempts){
        logger::log_debug(glue::glue("Maximum attempts to find neighborhood met, could not find: {nodes}"))
  ...
  # TODO: compute in-degree for each node in nodes. unclear to me is cites is
  #   collective across all nodes or somehow broken down at the individual node
  #   level. the following is wrong!!
  purrr::map_int(cites$link$pubmed_pubmed_citedin, \(x) length(unique(x)))
  
  # TODO: compute out-degree for each node in nodes
  
  # note that the order of the degree sequences must match the order of
  # the input vector `nodes`
...
```{r}
node_degrees(graph, good_node_ids[1])  # test with a single node
node_degrees(graph, good_node_ids)     # test with multiple nodes! this is the key one!
```
...
# important if you use the logging format i did above to prevent
# errors with glue::glue() and logging
