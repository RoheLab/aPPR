...
get_pubmed_data <- function(graph, node) {
  ...
  for (i in 1:graph$max_attempts) {
    ...
    logger::log_trace(glue::glue("Attempt {i}/{graph$max_attempts} to get node degrees: {node}"))
...
check.pubmed <- function(graph, nodes) {
  ...
  for (node in nodes) {
    ...
    if (node_data$num_refs + node_data$num_citedby > 0) {
      logger::log_trace(glue::glue("Checked node: {node} (good)"))
      good_nodes <- c(good_nodes, node)
      next
    }
    
    logger::log_trace(glue::glue("Checked node: {node} (bad)"))
  }
...
node_degrees.pubmed <- function(graph, nodes) {
  ...
  for (i in seq_along(nodes)) {
    logger::log_debug(glue::glue("Getting node degrees for node: {nodes[i]}"))
    ...
    logger::log_trace(glue::glue("In-degree for node: {degrees$in_degree[i]}"))
    logger::log_trace(glue::glue("Out-degree for node: {degrees$out_degree[i]}"))
  }
...
neighborhood.pubmed <- function(graph, node) {
  ...
  node_data <- memo_get_pubmed_data(graph, node)
  logger::log_debug(glue::glue("Done getting neighborhood: {node}"))
